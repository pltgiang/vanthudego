...
# 2. Add Tab UI
tab_html = "<div className={`folder-tab ${tab === 'position' ? 'active' : ''}`} onClick={() => setTab('position')}>Vị trí công việc</div>"
content = content.replace(
    tab_html,
...
render_html = "{tab === 'position' && <TabPositions ref={tabRef} can={can} onEdit={(item: any) => navigate('/job-positions/' + item.id)} />}"
content = content.replace(
    render_html,
    render_html + "\n          {tab === 'department' && <TabDepartments ref={tabRef} can={can} />}"
)

# 4. Append TabDepartments and DepartmentFormModal
new_components = """
